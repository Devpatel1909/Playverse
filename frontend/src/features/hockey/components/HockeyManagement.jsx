...
import hockeyAPIService from '../../../services/hockeyAPI';
...
const HockeyManagement = () => {
  ...
  const [teams, setTeams] = useState([
    {
      ...
      name: 'Mighty Ducks',
      shortName: 'MD',
      captain: 'Charlie Conway',
      coach: 'Gordon Bombay',
      homeGround: 'Eden Hall Arena',
      contactEmail: 'info@mightyducks.com',
      contactPhone: '+1-612-555-1234',
      established: '1992',
      players: [
        { id: 1, name: 'Charlie Conway', role: 'Forward', age: 28, jerseyNumber: 96 },
        { id: 2, name: 'Adam Banks', role: 'Forward', age: 29, jerseyNumber: 99 },
        { id: 3, name: 'Goldberg', role: 'Goalie', age: 30, jerseyNumber: 33 }
      ]
    },
    {
      id: 2,
      ...
      captain: 'Rick Riley',
      coach: 'Jack Reilly',
      homeGround: 'Minneapolis Arena',
      contactEmail: 'info@icehawks.com',
      contactPhone: '+1-612-555-5678',
      established: '1990',
      players: [
        { id: 4, name: 'Rick Riley', role: 'Forward', age: 30, jerseyNumber: 10 },
        { id: 5, name: 'Larson', role: 'Defenseman', age: 31, jerseyNumber: 22 },
        { id: 6, name: 'McGill', role: 'Forward', age: 30, jerseyNumber: 7 }
      ]
    },
  ]);
  ...
  const loadTeamsFromAPI = async () => {
    try {
      setError(null);
      const res = await hockeyAPIService.getAllTeams();
  ...
  const saveTeamsToStorage = async (newTeams) => {
    try {
      localStorage.setItem('hockeyTeams', JSON.stringify(newTeams));
      window.dispatchEvent(new CustomEvent('hockeyTeamsUpdated'));
    } catch (error) {
  ...
  const saveSubAdminsToStorage = (newSubAdmins) => {
    try {
      localStorage.setItem('hockeySubAdmins', JSON.stringify(newSubAdmins));
    } catch (error) {
  ...
  const updatePlayerStats = (playerId) => {
    const newGoals = Math.floor(Math.random() * 3); // Removed +1 to make it 0-2 goals
    const newAssists = Math.floor(Math.random() * 4); // Removed +1 to make it 0-3 assists
  ...
  const handleAddTeam = async () => {
    ...
    try {
      ...
      hockeyAPIService.validateTeamData(newTeam);
      const response = await hockeyAPIService.createTeam(newTeam);
  ...
  const handleAddPlayer = async () => {
    ...
    if (selectedTeam.players && selectedTeam.players.length >= 20) {
      alert('⚠️ Maximum Team Size Reached!\n\nEach team can have a maximum of 20 players. Please remove a player before adding a new one.');
      return;
    }
    ...
    try {
      ...
      hockeyAPIService.validatePlayerData(playerData);
      const response = await hockeyAPIService.addPlayer(selectedTeam._id || selectedTeam.id, playerData);
            ...
            <div className="flex flex-col justify-center flex-1">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="p-2 text-center border rounded bg-gradient-to-r from-purple-500/10 to-violet-600/10 border-purple-500/30">
                  ...
                  <div className={`text-xs font-bold ${ (team.players?.length || 0) >= 20 ? 'text-red-400' : (team.players?.length || 0) >= 15 ? 'text-yellow-400' : 'text-white'}`}>
  ...
  const PlayerCard = ({ player }) => {
    const getRoleColor = (role) => {
      ...
      if (role?.includes('Defenseman')) return 'from-blue-500 to-cyan-600';
      if (role?.includes('Goalie')) return 'from-red-500 to-rose-600';
      return 'from-gray-500 to-slate-600';
    };

    const getRoleIcon = (role) => {
      if (role?.includes('Forward')) return Target;
      if (role?.includes('Defenseman')) return Shield;
      if (role?.includes('Goalie')) return Zap;
      return User;
    };
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            ...
            <div className="p-2 text-center border rounded-lg bg-gradient-to-r from-yellow-500/10 to-amber-600/10 border-yellow-500/20">
              <div className="flex items-center justify-center mb-0.5">
                ...
                <div className="text-sm font-bold text-yellow-400">{player.assists || 0}</div>
              </div>
              <div className="text-xs font-medium text-white/60">Assists</div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between max-w-full">
                ...
                <div className="flex items-center space-x-2 lg:space-x-3">
                  ...
                  <button
                    ...
                    disabled={activeView === 'team-details' && selectedTeam && selectedTeam.players?.length >= 20}
                    className={`flex items-center px-3 py-2 space-x-1.5 transition-all duration-300 transform shadow-lg rounded-lg hover:scale-105 text-sm font-semibold ${ activeView === 'team-details' && selectedTeam && selectedTeam.players?.length >= 20 
                  ...
                  >
                    ...
                    <span className="text-white">
                      {activeView === 'teams' ? 'Create Team' : 
                       ...
                       selectedTeam && selectedTeam.players?.length >= 20 ? `Full (${selectedTeam.players?.length || 0}/20)` : `Add Player (${selectedTeam?.players?.length || 0}/20)`}
                    </span>
                  </button>
                </div>
              </div>
            ...
            {filteredTeams.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Create your first hockey team to get started'}
                </p>
              </div>
            ) : (
          ...
          <div>
            {filteredSubAdmins.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Add your first hockey sub-administrator to get started'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              ...
              <div className="relative p-4 border bg-gradient-to-r from-slate-900/95 via-slate-800/95 to-slate-900/95 backdrop-blur-xl rounded-xl border-white/20">
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="p-3 text-center border rounded-lg bg-gradient-to-r from-orange-500/10 to-red-600/10 border-orange-500/20">
                    ...
                    <p className={`text-sm font-bold ${ selectedTeam.players?.length >= 20 ? 'text-red-400' : selectedTeam.players?.length >= 15 ? 'text-yellow-400' : 'text-white'}`}>
                      {selectedTeam.players?.length || 0}/20 Players
                    </p>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className={`px-2.5 py-1 rounded-full text-xs font-medium border ${ selectedTeam.players?.length >= 20 
                      ? 'bg-red-500/20 text-red-300 border-red-500/30' 
                      : selectedTeam.players?.length >= 15 
                        ? 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30'
                        : 'bg-green-500/20 text-green-300 border-green-500/30' 
                  }`}>
                    {selectedTeam.players?.length || 0}/20 Squad Limit
                  </div>
                </div>
              </div>
              ...
              <div className="relative bg-gradient-to-br from-slate-900/98 via-slate-800/98 to-slate-900/98 backdrop-blur-2xl border border-white/20 rounded-xl p-5 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white">Create New Hockey Team</h2>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div className="relative w-full max-w-3xl">
              ...
              <div className="relative bg-gradient-to-br from-slate-900/98 via-slate-800/98 to-slate-900/98 backdrop-blur-2xl border border-white/20 rounded-xl p-5 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className={`mt-0.5 px-2 py-0.5 rounded-full text-xs font-medium inline-block border ${ selectedTeam.players?.length >= 20 
                          ? 'bg-red-500/20 text-red-300 border-red-500/30' 
                          : selectedTeam.players?.length >= 15 
                            ? 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30'
                            : 'bg-green-500/20 text-green-300 border-green-500/30' 
                      }`}>
                        Current Squad: {selectedTeam.players?.length || 0}/20 Players
                      </div>
                    </div>
                  </div>
                  <button
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value="Defenseman" className="bg-slate-800">Defenseman</option>
                        <option value="Goalie" className="bg-slate-800">Goalie</option>
                      </select>
                    </div>
                  </div>
                  <div>
                ...
                {selectedTeam.players?.length >= 15 && (
                  <div className={`mt-3 mb-4 p-3 rounded-lg border ${ selectedTeam.players?.length >= 20 
                      ...
                      <div>
                        {selectedTeam.players?.length >= 20 ? (
                          <div>
                            ...
                            <p className="text-xs opacity-80">This team already has 20 players (maximum allowed). Remove a player before adding new ones.</p>
                          </div>
                        ) : (
                          <div>
                            <h4 className="text-sm font-semibold">Approaching Squad Limit</h4>
                            <p className="text-xs opacity-80">Only {20 - (selectedTeam.players?.length || 0)} more player(s) can be added to reach the 20-player limit.</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="flex pt-4 space-x-3 border-t border-white/20">
                  <button
                    ...
                    disabled={selectedTeam.players?.length >= 20 || loading}
                    className={`flex-1 py-2.5 text-sm font-bold text-white transition-all duration-300 transform shadow-lg rounded-lg hover:scale-105 ${ selectedTeam.players?.length >= 20 || loading
                        ? 'bg-gradient-to-r from-gray-600 to-gray-700 cursor-not-allowed opacity-60'
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="Enter full name (e.g., Wayne Gretzky)"
                            required
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="admin@hockey.sports.com"
                            required
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Create, edit, and manage hockey teams</p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Schedule and manage hockey matches</p>
                            </div>
                          </div>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                    ...
                    >
                      {loading ? (
                        <div className="flex items-center justify-center space-x-2.5">
                          ...
                          <span>Creating Hockey Sub-Administrator...</span>
                        </div>
                      ) : (
                        <div className="flex items-center justify-center space-x-2.5">
                          <UserCheck className="w-5 h-5" />
                          <span>Create Hockey Sub-Administrator</span>
                        </div>
                      )}
                    </button>
                  ...
                  <div className="text-center">
                    <div className="w-20 h-20 mx-auto overflow-hidden border-2 rounded-lg bg-gradient-to-r from-slate-600 to-slate-700 border-white/20">
                      {playerPhotos[selectedPlayerForPhoto.id] ? (
                        <img 
                          src={playerPhotos[selectedPlayerForPhoto.id] ? (
                            playerPhotos[selectedPlayerForPhoto.id]
                          ) : (
                            ''
                          )}
