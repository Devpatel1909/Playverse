...
import tableTennisAPIService from '../../../services/tableTennisAPI';
...
const TableTennisManagement = () => {
  ...
  const [teams, setTeams] = useState([
    {
      ...
      captain: 'Ma Long',
      coach: 'Liu Guoliang',
      homeGround: 'China National Table Tennis Training Center',
      contactEmail: 'info@dragonwarriors.com',
      contactPhone: '+86 10 1234 5678',
      established: '2020',
      players: [
        { id: 1, name: 'Ma Long', role: 'Singles', age: 34, jerseyNumber: 1 },
        { id: 2, name: 'Fan Zhendong', role: 'Singles', age: 26, jerseyNumber: 2 },
        { id: 3, name: 'Xu Xin', role: 'Doubles', age: 33, jerseyNumber: 3 }
      ]
    },
    {
      id: 2,
      _id: '2',
      name: 'Samurai Spinners',
      shortName: 'SS',
      captain: 'Tomokazu Harimoto',
      coach: 'Yosuke Kurashima',
      homeGround: 'Ajinomoto National Training Center',
      contactEmail: 'info@samuraispinners.com',
      contactPhone: '+81 3-5963-0200',
      established: '2021',
      players: [
        { id: 4, name: 'Tomokazu Harimoto', role: 'Singles', age: 19, jerseyNumber: 4 },
        { id: 5, name: 'Mima Ito', role: 'Singles', age: 22, jerseyNumber: 5 },
        { id: 6, name: 'Jun Mizutani', role: 'Doubles', age: 33, jerseyNumber: 6 }
      ]
    },
  ]);
  ...
  const loadTeamsFromAPI = async () => {
    try {
      setError(null);
      const res = await tableTennisAPIService.getAllTeams();
  ...
  const saveTeamsToStorage = async (newTeams) => {
    try {
      localStorage.setItem('tableTennisTeams', JSON.stringify(newTeams));
      window.dispatchEvent(new CustomEvent('tableTennisTeamsUpdated'));
    } catch (error) {
  ...
  const saveSubAdminsToStorage = (newSubAdmins) => {
    try {
      localStorage.setItem('tableTennisSubAdmins', JSON.stringify(newSubAdmins));
    } catch (error) {
  ...
  const updatePlayerStats = (playerId) => {
    const newPointsWon = Math.floor(Math.random() * 11) + 5;
    const newMatchesWon = Math.floor(Math.random() * 2);
      ...
      players: team.players.map(player => 
        player.id === playerId 
          ? {
              ...
              pointsWon: (player.pointsWon || 0) + newPointsWon,
              matchesWon: (player.matchesWon || 0) + newMatchesWon,
            }
          : player
      )
    }));
  ...
  const handleAddTeam = async () => {
    ...
    try {
      ...
      tableTennisAPIService.validateTeamData(newTeam);
      const response = await tableTennisAPIService.createTeam(newTeam);
  ...
  const handleAddPlayer = async () => {
    ...
    try {
      ...
      tableTennisAPIService.validatePlayerData(playerData);
      const response = await tableTennisAPIService.addPlayer(selectedTeam._id || selectedTeam.id, playerData);
  ...
  const PlayerCard = ({ player }) => {
    const getRoleColor = (role) => {
      if (role?.includes('Singles')) return 'from-green-500 to-emerald-600';
      if (role?.includes('Doubles')) return 'from-blue-500 to-cyan-600';
      return 'from-gray-500 to-slate-600';
    };

    const getRoleIcon = (role) => {
      if (role?.includes('Singles')) return User;
      if (role?.includes('Doubles')) return Users;
      return User;
    };
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            <div className="p-2 text-center border rounded-lg bg-gradient-to-r from-green-500/10 to-emerald-600/10 border-green-500/20">
              <div className="flex items-center justify-center mb-0.5">
                ...
                <div className="text-sm font-bold text-green-400">{player.pointsWon || 0}</div>
              </div>
              <div className="text-xs font-medium text-white/60">Points Won</div>
            </div>
          </div>
            ...
            {filteredTeams.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Create your first table tennis team to get started'}
                </p>
              </div>
            ) : (
          ...
          <div>
            {filteredSubAdmins.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Add your first table tennis sub-administrator to get started'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        {activeView === 'team-details' && selectedTeam && !loading && (
          <div>
            ...
            <div className="relative mb-6">
              ...
              <div className="relative p-4 border rounded-xl bg-gradient-to-r from-slate-900/95 via-slate-800/95 to-slate-900/95 backdrop-blur-xl border-white/20">
              ...
              <div className="relative bg-gradient-to-br from-slate-900/98 via-slate-800/98 to-slate-900/98 backdrop-blur-2xl border border-white/20 rounded-xl p-5 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white">Create New Table Tennis Team</h2>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-transparent bg-gradient-to-r from-purple-400 via-pink-500 to-cyan-400 bg-clip-text">
                        Add Table Tennis Sub-Administrator
                      </h2>
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="Enter full name (e.g., Ma Long)"
                            required
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="admin@tabletennis.sports.com"
                            required
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Create, edit, and manage table tennis teams</p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Schedule and manage table tennis matches</p>
                            </div>
                          </div>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                    ...
                    >
                      {loading ? (
                        <div className="flex items-center justify-center space-x-2.5">
                          ...
                          <span>Creating Table Tennis Sub-Administrator...</span>
                        </div>
                      ) : (
                        <div className="flex items-center justify-center space-x-2.5">
                          <UserCheck className="w-5 h-5" />
                          <span>Create Table Tennis Sub-Administrator</span>
                        </div>
                      )}
                    </button>
                  ...
                  <div className="p-3 text-center border rounded-lg bg-slate-800/30 border-white/10">
                    <div className="space-y-1 text-xs text-slate-400">
                      ...
                      <div className="pt-1.5 mt-1.5 border-t border-slate-700">
                        Administrator: Dsp2810 | Created: 2025-08-24 12:35:01 | Status: Secure Registration
                      </div>
                    </div>
                  </div>
                </div>
