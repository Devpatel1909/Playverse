...
const TableTennisLiveScoringUI = () => {
    ...
    const fetchTableTennisScores = async () => {
      try {
        const [live, recent, upcoming] = await Promise.all([
          tableTennisAPIService.getLiveMatches(),
          tableTennisAPIService.getRecentMatches(),
          tableTennisAPIService.getUpcomingMatches(),
        ]);
    ...
    <Card className="mb-4 border-l-4 border-l-orange-500">
      <CardHeader className="pb-2">
        ...
        <div className="flex justify-between items-center mt-2">
          <span className="text-sm font-medium text-orange-600">{match.round}</span>
              ...
              <div className="flex items-center space-x-2">
                {match.sets1.map((set, index) => (
                  <div key={index} className={`w-8 h-8 flex items-center justify-center rounded ${
                    index === match.currentSet - 1 ? 'bg-orange-100 border-2 border-orange-500' : 'bg-gray-100'
                  }`}>
                    <span className="font-bold text-sm">{set}</span>
                  </div>
                ))}
                <div className="w-8 h-8 flex items-center justify-center rounded bg-orange-500 text-white">
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-orange-600 text-white py-6">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-orange-100 mt-2">Live matches, results, and upcoming tournaments</p>
        </div>
      </div>
...
export default TableTennisLiveScoringUI;
