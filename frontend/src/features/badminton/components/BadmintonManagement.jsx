...
import badmintonAPIService from '../../../services/badmintonAPI';
...
const BadmintonManagement = () => {
  ...
  const [teams, setTeams] = useState([
    {
      ...
      homeGround: 'Copenhagen Arena',
      contactEmail: 'info@shuttlemasters.com',
      contactPhone: '+45 1234 5678',
      established: '2023',
      players: [
        { id: 1, name: 'Viktor Axelsen', role: 'Singles', age: 29, jerseyNumber: 1 },
        { id: 2, name: 'Tai Tzu-ying', role: 'Singles', age: 28, jerseyNumber: 1 },
        { id: 3, name: 'Fajar Alfian', role: 'Doubles', age: 28, jerseyNumber: 7 }
      ]
    },
    {
      id: 2,
      ...
      captain: 'An Se-young',
      coach: 'Sung Ji-hyun',
      homeGround: 'Seoul National University Gymnasium',
      contactEmail: 'info@netdominators.com',
      contactPhone: '+82 2-880-5114',
      established: '2022',
      players: [
        { id: 4, name: 'An Se-young', role: 'Singles', age: 21, jerseyNumber: 1 },
        { id: 5, name: 'Loh Kean Yew', role: 'Singles', age: 25, jerseyNumber: 12 },
        { id: 6, name: 'Muhammad Rian Ardianto', role: 'Doubles', age: 27, jerseyNumber: 8 }
      ]
    },
  ]);
  ...
  const loadTeamsFromAPI = async () => {
    try {
      setError(null);
      const res = await badmintonAPIService.getAllTeams();
  ...
  const saveTeamsToStorage = async (newTeams) => {
    try {
      localStorage.setItem('badmintonTeams', JSON.stringify(newTeams));
      window.dispatchEvent(new CustomEvent('badmintonTeamsUpdated'));
    } catch (error) {
  ...
  const saveSubAdminsToStorage = (newSubAdmins) => {
    try {
      localStorage.setItem('badmintonSubAdmins', JSON.stringify(newSubAdmins));
    } catch (error) {
  ...
  const updatePlayerStats = (playerId) => {
    const newPoints = Math.floor(Math.random() * 21) + 10;
    const newSmashes = Math.floor(Math.random() * 15) + 5;
    const newNetErrors = Math.floor(Math.random() * 5);
      ...
      players: team.players.map(player => 
        player.id === playerId 
          ? {
              ...
              smashes: (player.smashes || 0) + newSmashes,
              netErrors: (player.netErrors || 0) + newNetErrors,
            }
          : player
      )
    }));
  ...
  const handleAddTeam = async () => {
    ...
    try {
      ...
      badmintonAPIService.validateTeamData(newTeam);
      const response = await badmintonAPIService.createTeam(newTeam);
  ...
  const handleAddPlayer = async () => {
    ...
    try {
      ...
      badmintonAPIService.validatePlayerData(playerData);
      const response = await badmintonAPIService.addPlayer(selectedTeam._id || selectedTeam.id, playerData);
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            ...
            <div className="p-2 text-center border rounded-lg bg-gradient-to-r from-green-500/10 to-emerald-600/10 border-green-500/20">
              <div className="flex items-center justify-center mb-0.5">
                ...
                <div className="text-sm font-bold text-green-400">{player.points || 0}</div>
              </div>
              <div className="text-xs font-medium text-white/60">Points</div>
            </div>
            <div className="p-2 text-center border rounded-lg bg-gradient-to-r from-yellow-500/10 to-amber-600/10 border-yellow-500/20">
              <div className="flex items-center justify-center mb-0.5">
                ...
                <div className="text-sm font-bold text-yellow-400">{player.smashes || 0}</div>
              </div>
              <div className="text-xs font-medium text-white/60">Smashes</div>
            </div>
          </div>
                  ...
                  <div className="flex items-center flex-1 min-w-0 space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h1 className="mb-0.5 text-xl font-bold text-transparent bg-gradient-to-r from-emerald-400 via-blue-500 to-purple-500 bg-clip-text lg:text-2xl xl:text-3xl truncate">
                        {activeView === 'teams' ? 'Badminton Management Hub' : 
                         activeView === 'sub-admins' ? 'Badminton Sub-Administrators' :
                      ...
                      <p className="text-sm font-medium truncate text-white/80 lg:text-base">
                        {activeView === 'teams' ? 'Professional Team & Sub-Admin Management System' : 
                         activeView === 'sub-admins' ? 'Manage Badminton Sub-Administrator Access & Permissions' :
            ...
            {filteredTeams.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Create your first badminton team to get started'}
                </p>
              </div>
            ) : (
          ...
          <div>
            {filteredSubAdmins.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Add your first badminton sub-administrator to get started'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              ...
              <div className="relative bg-gradient-to-br from-slate-900/98 via-slate-800/98 to-slate-900/98 backdrop-blur-2xl border border-white/20 rounded-xl p-5 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white">Create New Badminton Team</h2>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-transparent bg-gradient-to-r from-purple-400 via-pink-500 to-cyan-400 bg-clip-text">
                        Add Badminton Sub-Administrator
                      </h2>
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="Enter full name (e.g., Viktor Axelsen)"
                            required
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            placeholder="admin@badminton.sports.com"
                            required
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Create, edit, and manage badminton teams</p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between p-3 transition-colors border rounded-lg bg-slate-700/30 border-white/10 hover:bg-slate-600/30">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400">Schedule and manage badminton matches</p>
                            </div>
                          </div>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                    ...
                    >
                      {loading ? (
                        <div className="flex items-center justify-center space-x-2.5">
                          ...
                          <span>Creating Badminton Sub-Administrator...</span>
                        </div>
                      ) : (
                        <div className="flex items-center justify-center space-x-2.5">
                          <UserCheck className="w-5 h-5" />
                          <span>Create Badminton Sub-Administrator</span>
                        </div>
                      )}
                    </button>
