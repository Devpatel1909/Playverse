...
const BasketballManagement = () => {
  ...
  const [teams, setTeams] = useState([
    {
      ...
      captain: 'Arjun Patel',
      coach: 'Mike Johnson',
      homeGround: 'Sports Complex Arena',
      contactEmail: 'info@ahmedabadballers.com',
      contactPhone: '+91-9876543210',
      established: '2020',
      players: [
        { id: 1, name: 'Arjun Patel', role: 'Captain/Point Guard', age: 24, jerseyNumber: 1, height: '6\'2"' },
        { id: 2, name: 'Karan Singh', role: 'Center', age: 26, jerseyNumber: 5, height: '6\'8"' },
        { id: 3, name: 'Rohit Sharma', role: 'Forward', age: 25, jerseyNumber: 23, height: '6\'5"' }
      ]
    },
    {
      id: 2,
      ...
      coach: 'David Smith',
      homeGround: 'City Basketball Court',
      contactEmail: 'info@vadodarahoops.com',
      contactPhone: '+91-9876543211',
      established: '2019',
      players: [
        { id: 4, name: 'Vikash Kumar', role: 'Captain/Shooting Guard', age: 23, jerseyNumber: 3, height: '6\'3"' },
        { id: 5, name: 'Amit Joshi', role: 'Power Forward', age: 27, jerseyNumber: 4, height: '6\'7"' }
      ]
    }
  ]);
  ...
  const [subAdmins, setSubAdmins] = useState([
    {
      ...
      specialization: 'Score Management',
      status: 'Active',
      permissions: {
        manageTeams: true,
        managePlayers: true,
  ...
  const PlayerCard = ({ player }) => {
    const getRoleColor = (role) => {
      ...
      if (role?.includes('Point Guard')) return 'from-green-500 to-emerald-600';
      if (role?.includes('Shooting Guard')) return 'from-blue-500 to-cyan-600';
      if (role?.includes('Forward')) return 'from-red-500 to-rose-600';
      if (role?.includes('Center')) return 'from-purple-500 to-violet-600';
      return 'from-gray-500 to-slate-600';
    };

    const getRoleIcon = (role) => {
      if (role?.includes('Captain')) return Crown;
      if (role?.includes('Point Guard')) return Target;
      if (role?.includes('Shooting Guard')) return Zap;
      if (role?.includes('Forward')) return Shield;
      if (role?.includes('Center')) return Award;
      return User;
    };
                  ...
                  <div className="flex items-center flex-1 min-w-0 space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h1 className="mb-0.5 text-xl font-bold text-transparent bg-gradient-to-r from-emerald-400 via-blue-500 to-purple-500 bg-clip-text lg:text-2xl xl:text-3xl truncate">
                        {activeView === 'teams' ? 'Basketball Management Hub' : 
                         activeView === 'sub-admins' ? 'Basketball Sub-Administrators' :
                      ...
                      <p className="text-sm font-medium truncate text-white/80 lg:text-base">
                        {activeView === 'teams' ? 'Professional Team & Sub-Admin Management System' : 
                         activeView === 'sub-admins' ? 'Manage Basketball Sub-Administrator Access & Permissions' :
        ...
        {loading && (
          <div className="flex items-center justify-center py-16">
            <div className="text-center">
              ...
              <p className="text-base font-medium text-white/70">Loading basketball data...</p>
            </div>
          </div>
        )}
            ...
            {filteredTeams.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Create your first basketball team to get started'}
                </p>
              </div>
            ) : (
          ...
          <div>
            {filteredSubAdmins.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-sm text-white/40">
                  {searchTerm ? 'Try adjusting your search terms' : 'Add your first basketball sub-administrator to get started'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
