// Hockey API Service for Frontend
const API_BASE_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api';

class HockeyAPIService {
  constructor() {
    this.baseURL = `${API_BASE_URL}/hockey`;
  }
  ...
  async getAllTeams() {
    ...
    try {
      if (import.meta.env.DEV) console.debug('[HockeyAPI] GET', url);
  ...
  async createSubadmin(teamId, creds) {
    try {
      const res = await fetch(`${this.baseUrl}/hockey/teams/${teamId}/subadmins`, {
        method: 'POST',
...
const hockeyAPIService = new HockeyAPIService();
export default hockeyAPIService;
