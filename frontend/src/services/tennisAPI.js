// Tennis API Service for Frontend
const API_BASE_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api';

class TennisAPIService {
  constructor() {
    this.baseURL = `${API_BASE_URL}/tennis`;
  }
  ...
  async getAllTeams() {
    ...
    try {
      if (import.meta.env.DEV) console.debug('[TennisAPI] GET', url);
  ...
  validatePlayerData(playerData) {
    ...
    // Age validation
    if (playerData.age < 14 || playerData.age > 50) {
  ...
  async createSubadmin(teamId, creds) {
    try {
      const res = await fetch(`${this.baseUrl}/tennis/teams/${teamId}/subadmins`, {
        method: 'POST',
...
const tennisAPIService = new TennisAPIService();
