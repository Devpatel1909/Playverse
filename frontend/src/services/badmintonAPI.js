// Badminton API Service for Frontend
const API_BASE_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api';

class BadmintonAPIService {
  constructor() {
    this.baseURL = `${API_BASE_URL}/badminton`;
  }
  ...
  async getAllTeams() {
    ...
    try {
      if (import.meta.env.DEV) console.debug('[BadmintonAPI] GET', url);
  ...
  validatePlayerData(playerData) {
    ...
    // Age validation
    if (playerData.age < 14 || playerData.age > 45) {
      throw new Error('Player age must be between 14 and 45');
    }
  ...
  async createSubadmin(teamId, creds) {
    try {
      const res = await fetch(`${this.baseUrl}/badminton/teams/${teamId}/subadmins`, {
        method: 'POST',
...
const badmintonAPIService = new BadmintonAPIService();
export default badmintonAPIService;
