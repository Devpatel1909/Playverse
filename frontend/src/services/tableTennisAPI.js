// Table Tennis API Service for Frontend
const API_BASE_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api';

class TableTennisAPIService {
  constructor() {
    this.baseURL = `${API_BASE_URL}/tabletennis`;
  }
  ...
  async getAllTeams() {
    ...
    try {
      if (import.meta.env.DEV) console.debug('[TableTennisAPI] GET', url);
  ...
  validatePlayerData(playerData) {
    ...
    // Age validation
    if (playerData.age < 10 || playerData.age > 50) {
      throw new Error('Player age must be between 10 and 50');
    }
  ...
  async createSubadmin(teamId, creds) {
    try {
      const res = await fetch(`${this.baseUrl}/tabletennis/teams/${teamId}/subadmins`, {
        method: 'POST',
...
const tableTennisAPIService = new TableTennisAPIService();
export default tableTennisAPIService;
